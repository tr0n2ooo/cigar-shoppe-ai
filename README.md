...
- Minimum Level and Reorder Quantity spreadsheet fields removed from all reorder logic — replaced by velocity-derived computed fields:
  - `velocity_min_level` — 2-week safety stock at current velocity
  - `velocity_reorder_qty` — 30-day supply at current velocity
- `markup_pct` replaced by `margin_pct` computed from first principles: `(selling_price − cost) / selling_price × 100`
- Thread-safe DuckDB: `threading.Lock` guards all `.execute()` calls in `tools/inventory_tool.py`, fixing a `NoneType.iterrows()` crash that occurred when parallel tools shared the same connection

**Data & house brand logic**
- ASW → SS rename in all 9 item descriptions (e.g. "ASW Conn. Toro" → "SS Conn. Toro")
- House brand exclusion narrowed: only `Brand = "Smoke Shoppe"` (the SS Conn. line) skips external research — "Signature House Blend" is now treated as a regular third-party cigar
- Custom logo fix: `public/logo_dark.svg` / `public/logo_light.svg` added so Chainlit's `/logo` endpoint no longer falls back to the default
...
### v0.7 (2026-06-06)
Natural-language dispatcher (`dispatcher_agent.py`) routing chat messages to all specialist tools. Auto-discovers every `*_server.py` tool via FastMCP introspection. Added cigar research + social reputation + inventory stock lookups to chat UI. Password auth via Chainlit. Docker packaging. Smoke Shoppe house brand handling.
...
└── public/
    ...
    ├── logo_dark.svg          # Chainlit /logo endpoint (dark theme) — required to suppress warning
    ├── logo_light.svg         # Chainlit /logo endpoint (light theme)
    └── logo-option-3-wordmark.svg  # source wordmark
```
...
| Tool | Description |
|------|-------------|
| `get_reorder_signals` | OOS and stockout-risk items; velocity-driven only (Minimum Level / Reorder Quantity fields ignored); outputs `velocity_min_level` and `velocity_reorder_qty` as computed fields |
| `get_slow_movers` | Excess stock with low velocity — discount candidates with seasonality context; includes `margin_pct` |
| `get_discontinue_candidates` | Dead stock with ≤ 2 YTD units — back-half seasonal items automatically excluded |
| `get_top_profitable` | Top items by YTD gross profit with stock adequacy indicator and `margin_pct` |
| `get_full_inventory_report` | All four analyses in one call |
| `search_inventory_by_name` | Search live inventory by name/brand fragment — returns on-hand qty, price, cost for all matching SKUs |
...
```bash
docker build --platform linux/amd64 -t cigar-shoppe-ai:0.8 .
docker save cigar-shoppe-ai:0.8 | gzip > cigar-shoppe-ai-0.8.tar.gz
```

### Transfer to NAS
```bash
scp cigar-shoppe-ai-0.8.tar.gz user@nas-ip:/volume1/docker/cigar-shoppe/
...
docker load -i cigar-shoppe-ai-0.8.tar.gz
